...
curl -OL "https://github.com/temporalio/cli/releases/download/v${TEMPORAL_CLI_VERSION}/temporal_cli_${TEMPORAL_CLI_VERSION}_linux_${TARGET_GOARCH}.tar.gz"
tar -xzf "temporal_cli_${TEMPORAL_CLI_VERSION}_linux_${TARGET_GOARCH}.tar.gz"
mv temporal /usr/bin/temporal
chmod +x /usr/bin/temporal
rm -f "temporal_cli_${TEMPORAL_CLI_VERSION}_linux_${TARGET_GOARCH}.tar.gz"

# Install Temporal Server
echo "=== Installing Temporal Server v${TEMPORAL_SERVER_VERSION} ==="
curl -OL "https://github.com/temporalio/temporal/releases/download/v${TEMPORAL_SERVER_VERSION}/temporal_${TEMPORAL_SERVER_VERSION}_linux_${TARGET_GOARCH}.tar.gz"
tar -xzf "temporal_${TEMPORAL_SERVER_VERSION}_linux_${TARGET_GOARCH}.tar.gz"
mv temporal-server /usr/bin/temporal-server
chmod +x /usr/bin/temporal-server
rm -f "temporal_${TEMPORAL_SERVER_VERSION}_linux_${TARGET_GOARCH}.tar.gz"

# Install Temporal UI Server
echo "=== Installing Temporal UI Server v${TEMPORAL_UI_VERSION} ==="
curl -OL "https://github.com/temporalio/ui-server/releases/download/v${TEMPORAL_UI_VERSION}/ui-server_${TEMPORAL_UI_VERSION}_linux_${TARGET_GOARCH}.tar.gz"
tar -xzf "ui-server_${TEMPORAL_UI_VERSION}_linux_${TARGET_GOARCH}.tar.gz"
mv ui-server /usr/bin/temporal-ui-server
chmod +x /usr/bin/temporal-ui-server
rm -f "ui-server_${TEMPORAL_UI_VERSION}_linux_${TARGET_GOARCH}.tar.gz"
...
curl -OL "https://github.com/containifyci/temporal-worker/releases/download/v${TEMPORAL_WORKER_VERSION}/temporal-dunebot-worker_linux_${TARGET_GOARCH}"
mv temporal-dunebot-worker_linux_${TARGET_GOARCH} /usr/bin/temporal-dunebot-worker
